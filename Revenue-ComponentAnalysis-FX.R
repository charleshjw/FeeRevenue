...
#
#segment1 <- c("FeeBased","SpreadBased","CM");
...
#.frequency <- start_and_frequency$frequency;
#
#segment1 <- c("FXGlobalServices","NorthAmericaTrading","EMEATrading","APACTrading","Others");
#
#segment1 <- paste0("Revenue.",segment1)
#length(segment1) == sum(colnames(revenue_lob)%in%segment1);
#segmentation <- revenue_lob[,colnames(revenue_lob)%in%segment1];
...
segment1 <- c("FeeBased","Payments","SpreadBased","ADR","CurrencyAdmin","FX_Other","CM");
