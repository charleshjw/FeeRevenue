...
EXECUTE_REG_ANALYSIS_3V <- FALSE;
EXECUTE_DIFF_REG_ANALYSIS <- TRUE;
...
Output_root <- "./Output-Q22016-Short2012/";
Time_periods <- 61:114;
...
#	segment_peroid_end <- c(2015,12);
	...
	segment_peroid_start_q <- c(2015,3);
#	segment_peroid_end <- c(2015,12);
	segment_peroid_end_q <- c(2016,2);
...
if (EXECUTE_REG_ANALYSIS_3V) {
	print("--Execute Regression Analysis with 3 Variables\n");
	External_variable_selection_list <- c("SP500_END","NASDAQTrades_AVE","SPVIX_END","RealGDPGrowth_END");
	EXECUTE_ALGO_1 <- TRUE;
	EXECUTE_ALGO_2 <- TRUE;
	source("./Revenue-AlgoRegression3V.R");
}
